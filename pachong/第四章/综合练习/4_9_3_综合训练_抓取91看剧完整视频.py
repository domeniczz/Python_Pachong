...
async def dec_ts(name, key):
    # IV的位数 要和key一样
    aes = AES.new(key=key, IV=b"0000000000000000", mode=AES.MODE_CBC, encoding="UTF-8")
...
def merge_ts():
    ...
    os.system(f"cat {s} > movie.mp4")  # mac系统用cat命令，这里相当于在命令行执行
    # 其实不需要一个个添加.ts文件，可以用通配符*.ts
    print("搞定!")
